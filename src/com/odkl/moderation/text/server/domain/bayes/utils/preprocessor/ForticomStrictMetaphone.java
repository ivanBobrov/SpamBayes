...
public class ForticomStrictMetaphone implements SpamTextPreprocessor {
    ...
    private String encodeMetaphone(StringBuilder string, RawStringType rawStringType) {
        ...
        switch (currentChar) {
            case 'Y': if (isNext(string, pos, 'I')) { result.append('I'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'E')) { result.append('J').append('I'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'A')) { result.append('J').append('A'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'U')) { result.append('J').append('U'); pos+=2; ++vowels;}
                break;
            case 'Э': result.append('I'); ++pos; break;
            case 'Я': result.append('J').append('A'); ++pos; ++vowels; break;
            case 'Ю': result.append('J').append('U'); ++pos; ++vowels; break;
            case 'Е': result.append('J').append('I'); ++pos; ++vowels; break;
            case 'Ё': result.append('J').append('O'); ++pos; ++vowels; break;
            case 'Й': result.append('J'); ++pos; break;
            case 'J': if (isNext(string, pos, 'E')) { result.append('J').append('I'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'A')) { result.append('J').append('A'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'U')) { result.append('J').append('U'); pos+=2; ++vowels;}
            else if (isNext(string, pos, 'O')) { result.append('J').append('O'); pos+=2; ++vowels;}
                break;
            case 'Д':
            case 'D': result.append('D'); ++pos; vowels = 0; break;
            case 'Г':
            case 'G': result.append('G'); ++pos; vowels = 0; break;
            case 'В':
            case 'V': result.append('V'); ++pos; vowels = 0; break;
        ...
        for (; pos < string.length() && result.length() < maxKeyLength; ++pos) {
            ...
            switch (currentChar) {
                /* Русские */
                case 'К':
                case 'Г': result.append('K'); break;
                case 'Н': result.append('N'); break;
                case '6': if (leaveDigits) result.append('6'); break;
                case 'Ш':
                case 'Щ': result.append('S'); break;
                case 'Х': result.append('H'); break;
                case 'Ф':
                case 'В': result.append('F'); break;
                case 'П': result.append('P'); break;
                case 'Р': result.append('R'); break;
                case 'Л': result.append('L'); break;
                case 'З': result.append('Z'); break;
                case 'Ж': result.append('Z').append('H'); break;
                case '4': if (leaveDigits) result.append('4'); break;
                case 'Ц':
                case 'Ч': result.append('C'); break;
                ...
                case 'J':
                    if (vowels < maxVowel) {
                        ++vowels;
                        if (isNext(string, pos, 'E')) { result.append('E'); ++pos; }
                        else if (isNext(string, pos, 'A')) { if (pos + 2 == string.length()) {result.append("JA"); ++vowels; } else {result.append('O');} ++pos; }
                        else if (isNext(string, pos, 'U')) { result.append('U'); ++pos; }
                        else if (isNext(string, pos, 'O')) { result.append('E'); ++pos; }
                        else if (pos + 1 < string.length()) { result.append('I'); }
                    }
                    break;
                case 'Q': result.append('K'); break;
                case 'W': result.append('F'); break;
                case 'R': result.append('R'); break;
                case 'T': if (isNext(string, pos, 'H')) { ++pos; } result.append('T'); break;
                case 'P': if (isNext(string, pos, 'H')) { result.append('F'); ++pos; } else { result.append('P'); } break;
                case 'S': result.append('S');
                    if (isNext(string, pos, 'C') && isNext(string, pos + 1, 'H')) pos += 2;
                    break;
                case 'D': result.append('T'); break;
                case 'F': result.append('F'); break;
                case 'G': if (isNext(string, pos, 'H')) { ++pos; } result.append('K'); break;
                case 'H':
                    if (cyrillic) {
                        result.append('N');
                    ...
                    break;
                case 'K': result.append('K'); break;
                case 'L': result.append('L'); break;
                case 'Z': result.append('Z'); if (isNext(string, pos, 'H')) { result.append('H'); ++pos; }  break;
                ...
                case 'N': if (isNext(string, pos, 'H')) { ++pos; } result.append('N'); break;
                ...
                default :
                    if (leaveDigits && Character.isDigit(currentChar)) {
                        ...
                        switch (currentChar) {
                    /**/case 'Ё': result.append('E'); ++vowels; break;
                    /**/case 'Е':
                    /**/case 'И': if (isNext(string, pos, 'Е')) {result.append("IE"); vowels+=2; ++pos; break; }
                            case 'E': if (isNext(string, pos, 'J')) {result.append("II"); vowels+=2; ++pos; break; }
                    /**/case 'Й': if (pos + 1 == string.length()) break;
                    /**/case 'І':
                            case 'I': result.append('I'); ++vowels; break;
                    /**/case 'У':
                    /**/case 'Ю':
                            case 'U': result.append('U'); ++vowels; break;
                    /**/case 'Я': if (pos + 1 == string.length()) { result.append("JA"); vowels+=2; break; }
                    /**/case 'А':
                            case 'A': result.append(pos + 1 == string.length() ? 'A' : 'O'); ++vowels; break;
                            case '0':
                    /**/case 'О':
                            case 'O': result.append('O'); ++vowels; break;
                            case 'Y': if (isNext(string, pos, 'A'))   {result.append(pos + 2 == string.length() ? "JA" : "O"); ++vowels; ++pos; }
                            else if (isNext(string, pos, 'E'))   {result.append('E'); ++vowels; ++pos; }
                            else if (isNext(string, pos, 'O'))   {result.append('E'); ++vowels;}
                            else if (pos + 1 == string.length()) {result.append('I'); ++vowels;}
                                break;
                        }
                    }
            }
